...
find_missing_dates_ss = function(weathframe) {
...
create_missingframe_ss = function(missingdates) {
...
combine_two_frames_ss = function(weathframe,missingframe) {
...
monthly_summary_from_daily_ss = function(dataframe) {
...
monthly_sansimon = function(dataframe) {
  # this function takes the raw data from the San Simon station and reduces it to monthly data
  ...
  missingdates = find_missing_dates_ss(weathframe)
  missingframe = create_missingframe_ss(as.Date(missingdates))
  withgaps = combine_two_frames_ss(weathframe,missingframe)
  monthly_ss = monthly_summary_from_daily_ss(withgaps)
  
  outfile = "data/Monthly_ppt_SanSimon.csv"
  write.csv(monthly_ss,file=outfile,row.names=F)
}

sansimonfile = "data/SanSimon_ppt.csv"
sansimonframe = csv_to_dataframe(sansimonfile)
monthly_sansimon(sansimonframe)
